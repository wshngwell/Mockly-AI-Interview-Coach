package com.example.interviewaicoach.presentation.screens.resultsScreen
...
import com.example.interviewaicoach.presentation.screens.destinations.ResultsScreenDestination
import com.example.interviewaicoach.presentation.screens.questionsWithAnswersScreen.BottomElementsBar
...
import com.example.interviewaicoach.presentation.viemodels.ResultScreenViewModel.Event
import com.example.interviewaicoach.presentation.viemodels.ResultScreenViewModel.Intent
...
fun ResultsScreen(
    ...
    val intent: (Intent) -> Unit by remember(Unit) {
    ...
    LaunchedEffect(Unit) {
        event.filterIsInstance<Event>().collect {
            when (it) {
                ...
                is Event.NavigateOnQuestionsScreenAndRetry -> {
...
private fun UI(
    ...
    Column(
        ...
        Column(
            ...
            FinalResultsCard(
                averageScore = if (state.countsOfQuestions == 0 || state.numberOfVoiceAnsweredQuestions == 0) 0 else state.pointsSum / state.numberOfVoiceAnsweredQuestions,
        ...
        BottomElementsBar(
            onRightButtonClicked = { intent(Intent.OnRetryButtonClicked) },
            onLeftButtonClicked = { intent(Intent.OnGoToMainScreenButtonClicked) },
